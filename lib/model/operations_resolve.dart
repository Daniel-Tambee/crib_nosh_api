...
class OperationsResolve {
  /// Returns a new [OperationsResolve] instance.
  OperationsResolve({
    this.id,
    this.name,
    this.key,
  ...
  @override
  bool operator ==(Object other) => identical(this, other) || other is OperationsResolve &&
    other.id == id &&
    other.name == name &&
    other.key == key &&
    other.type == type &&
  ...
  @override
  String toString() => 'OperationsResolve[id=$id, name=$name, key=$key, type=$type, positionX=$positionX, positionY=$positionY, options=$options, resolve=$resolve, reject=$reject, flow=$flow, dateCreated=$dateCreated, userCreated=$userCreated]';
  ...
  /// Returns a new [OperationsResolve] instance and imports its values from
  /// [value] if it's a [Map], null otherwise.
  // ignore: prefer_constructors_over_static_methods
  static OperationsResolve? fromJson(dynamic value) {
    if (value is Map) {
      ...
      assert(() {
        requiredKeys.forEach((key) {
          assert(json.containsKey(key), 'Required key "OperationsResolve[$key]" is missing from JSON.');
          assert(json[key] != null, 'Required key "OperationsResolve[$key]" has a null value in JSON.');
        });
        return true;
      }());

      return OperationsResolve(
  ...
  static List<OperationsResolve> listFromJson(dynamic json, {bool growable = false,}) {
    final result = <OperationsResolve>[];
    if (json is List && json.isNotEmpty) {
      for (final row in json) {
        final value = OperationsResolve.fromJson(row);
        if (value != null) {
          result.add(value);
        }
      }
  ...
  static Map<String, OperationsResolve> mapFromJson(dynamic json) {
    final map = <String, OperationsResolve>{};
    if (json is Map && json.isNotEmpty) {
      json = json.cast<String, dynamic>(); // ignore: parameter_assignments
      for (final entry in json.entries) {
        final value = OperationsResolve.fromJson(entry.value);
        if (value != null) {
          map[entry.key] = value;
        }
      }
  ...
  // maps a json object with a list of OperationsResolve-objects as value to a dart map
  static Map<String, List<OperationsResolve>> mapListFromJson(dynamic json, {bool growable = false,}) {
    final map = <String, List<OperationsResolve>>{};
    if (json is Map && json.isNotEmpty) {
      // ignore: parameter_assignments
      json = json.cast<String, dynamic>();
      for (final entry in json.entries) {
        map[entry.key] = OperationsResolve.listFromJson(entry.value, growable: growable,);
      }
    }
    return map;
  }
