...
class Roles {
  ...
  @override
  bool operator ==(Object other) => identical(this, other) || other is Roles &&
    other.id == id &&
    other.name == name &&
    other.icon == icon &&
  ...
  @override
  String toString() => 'Roles[id=$id, name=$name, icon=$icon, description=$description, parent=$parent, children=$children, policies=$policies, users=$users]';
    ...
    if (value is Map) {
      ...
      assert(() {
        requiredKeys.forEach((key) {
          assert(json.containsKey(key), 'Required key "Roles[$key]" is missing from JSON.');
          assert(json[key] != null, 'Required key "Roles[$key]" has a null value in JSON.');
        });
        return true;
      }());
  ...
  static List<Roles> listFromJson(dynamic json, {bool growable = false,}) {
    final result = <Roles>[];
  ...
  static Map<String, List<Roles>> mapListFromJson(dynamic json, {bool growable = false,}) {
    final map = <String, List<Roles>>{};
    if (json is Map && json.isNotEmpty) {
      // ignore: parameter_assignments
      json = json.cast<String, dynamic>();
      for (final entry in json.entries) {
        map[entry.key] = Roles.listFromJson(entry.value, growable: growable,);
      }
    }
    return map;
  }
