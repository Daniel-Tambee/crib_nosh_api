...
class CollectionsGroup {
  /// Returns a new [CollectionsGroup] instance.
  CollectionsGroup({
    this.collection,
    this.icon,
    this.note,
  ...
  @override
  bool operator ==(Object other) => identical(this, other) || other is CollectionsGroup &&
  ...
  @override
  String toString() => 'CollectionsGroup[collection=$collection, icon=$icon, note=$note, displayTemplate=$displayTemplate, hidden=$hidden, singleton=$singleton, translations=$translations, archiveField=$archiveField, archiveAppFilter=$archiveAppFilter, archiveValue=$archiveValue, unarchiveValue=$unarchiveValue, sortField=$sortField, accountability=$accountability, color=$color, itemDuplicationFields=$itemDuplicationFields, sort=$sort, group=$group, collapse=$collapse, previewUrl=$previewUrl, versioning=$versioning]';
  ...
  /// Returns a new [CollectionsGroup] instance and imports its values from
  /// [value] if it's a [Map], null otherwise.
  // ignore: prefer_constructors_over_static_methods
  static CollectionsGroup? fromJson(dynamic value) {
    if (value is Map) {
      ...
      assert(() {
        requiredKeys.forEach((key) {
          assert(json.containsKey(key), 'Required key "CollectionsGroup[$key]" is missing from JSON.');
          assert(json[key] != null, 'Required key "CollectionsGroup[$key]" has a null value in JSON.');
        });
        return true;
      }());
  ...
  static List<CollectionsGroup> listFromJson(dynamic json, {bool growable = false,}) {
    final result = <CollectionsGroup>[];
    if (json is List && json.isNotEmpty) {
      for (final row in json) {
        final value = CollectionsGroup.fromJson(row);
        if (value != null) {
          result.add(value);
        }
      }
  ...
  static Map<String, CollectionsGroup> mapFromJson(dynamic json) {
    final map = <String, CollectionsGroup>{};
    if (json is Map && json.isNotEmpty) {
      json = json.cast<String, dynamic>(); // ignore: parameter_assignments
      for (final entry in json.entries) {
        final value = CollectionsGroup.fromJson(entry.value);
        if (value != null) {
          map[entry.key] = value;
        }
      }
  ...
  // maps a json object with a list of CollectionsGroup-objects as value to a dart map
  static Map<String, List<CollectionsGroup>> mapListFromJson(dynamic json, {bool growable = false,}) {
    final map = <String, List<CollectionsGroup>>{};
    if (json is Map && json.isNotEmpty) {
      // ignore: parameter_assignments
      json = json.cast<String, dynamic>();
      for (final entry in json.entries) {
        map[entry.key] = CollectionsGroup.listFromJson(entry.value, growable: growable,);
      }
    }
    return map;
  }
