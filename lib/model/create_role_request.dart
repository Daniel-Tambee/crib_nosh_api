...
class CreateRoleRequest {
  ...
  @override
  String toString() => 'CreateRoleRequest[description=$description, enforceTfa=$enforceTfa, externalId=$externalId, ipAccess=$ipAccess, moduleListing=$moduleListing, name=$name]';
  ...
  /// Returns a new [CreateRoleRequest] instance and imports its values from
  /// [value] if it's a [Map], null otherwise.
  // ignore: prefer_constructors_over_static_methods
  static CreateRoleRequest? fromJson(dynamic value) {
    if (value is Map) {
      ...
      assert(() {
        requiredKeys.forEach((key) {
          assert(json.containsKey(key), 'Required key "CreateRoleRequest[$key]" is missing from JSON.');
          assert(json[key] != null, 'Required key "CreateRoleRequest[$key]" has a null value in JSON.');
        });
        return true;
      }());
  ...
  static List<CreateRoleRequest> listFromJson(dynamic json, {bool growable = false,}) {
    final result = <CreateRoleRequest>[];
    if (json is List && json.isNotEmpty) {
      for (final row in json) {
        final value = CreateRoleRequest.fromJson(row);
        if (value != null) {
          result.add(value);
        }
      }
  ...
  static Map<String, CreateRoleRequest> mapFromJson(dynamic json) {
    final map = <String, CreateRoleRequest>{};
    if (json is Map && json.isNotEmpty) {
      json = json.cast<String, dynamic>(); // ignore: parameter_assignments
      for (final entry in json.entries) {
        final value = CreateRoleRequest.fromJson(entry.value);
        if (value != null) {
          map[entry.key] = value;
        }
      }
  ...
  // maps a json object with a list of CreateRoleRequest-objects as value to a dart map
  static Map<String, List<CreateRoleRequest>> mapListFromJson(dynamic json, {bool growable = false,}) {
    final map = <String, List<CreateRoleRequest>>{};
    if (json is Map && json.isNotEmpty) {
      // ignore: parameter_assignments
      json = json.cast<String, dynamic>();
      for (final entry in json.entries) {
        map[entry.key] = CreateRoleRequest.listFromJson(entry.value, growable: growable,);
      }
    }
    return map;
  }
