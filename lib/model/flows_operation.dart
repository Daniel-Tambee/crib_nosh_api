...
class FlowsOperation {
  /// Returns a new [FlowsOperation] instance.
  FlowsOperation({
    this.id,
    this.name,
    this.key,
  ...
  @override
  bool operator ==(Object other) => identical(this, other) || other is FlowsOperation &&
    other.id == id &&
    other.name == name &&
    other.key == key &&
    other.type == type &&
  ...
  @override
  String toString() => 'FlowsOperation[id=$id, name=$name, key=$key, type=$type, positionX=$positionX, positionY=$positionY, options=$options, resolve=$resolve, reject=$reject, flow=$flow, dateCreated=$dateCreated, userCreated=$userCreated]';
  ...
  /// Returns a new [FlowsOperation] instance and imports its values from
  /// [value] if it's a [Map], null otherwise.
  // ignore: prefer_constructors_over_static_methods
  static FlowsOperation? fromJson(dynamic value) {
    if (value is Map) {
      ...
      assert(() {
        requiredKeys.forEach((key) {
          assert(json.containsKey(key), 'Required key "FlowsOperation[$key]" is missing from JSON.');
          assert(json[key] != null, 'Required key "FlowsOperation[$key]" has a null value in JSON.');
        });
        return true;
      }());

      return FlowsOperation(
  ...
  static List<FlowsOperation> listFromJson(dynamic json, {bool growable = false,}) {
    final result = <FlowsOperation>[];
    if (json is List && json.isNotEmpty) {
      for (final row in json) {
        final value = FlowsOperation.fromJson(row);
        if (value != null) {
          result.add(value);
        }
      }
  ...
  static Map<String, FlowsOperation> mapFromJson(dynamic json) {
    final map = <String, FlowsOperation>{};
    if (json is Map && json.isNotEmpty) {
      json = json.cast<String, dynamic>(); // ignore: parameter_assignments
      for (final entry in json.entries) {
        final value = FlowsOperation.fromJson(entry.value);
        if (value != null) {
          map[entry.key] = value;
        }
      }
  ...
  // maps a json object with a list of FlowsOperation-objects as value to a dart map
  static Map<String, List<FlowsOperation>> mapListFromJson(dynamic json, {bool growable = false,}) {
    final map = <String, List<FlowsOperation>>{};
    if (json is Map && json.isNotEmpty) {
      // ignore: parameter_assignments
      json = json.cast<String, dynamic>();
      for (final entry in json.entries) {
        map[entry.key] = FlowsOperation.listFromJson(entry.value, growable: growable,);
      }
    }
    return map;
  }
