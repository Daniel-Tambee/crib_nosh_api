...
class UpdatePermissionsRequest {
  ...
  @override
  String toString() => 'UpdatePermissionsRequest[keys=$keys, data=$data]';
    ...
    if (value is Map) {
      ...
      assert(() {
        requiredKeys.forEach((key) {
          assert(json.containsKey(key), 'Required key "UpdatePermissionsRequest[$key]" is missing from JSON.');
          assert(json[key] != null, 'Required key "UpdatePermissionsRequest[$key]" has a null value in JSON.');
        });
        return true;
      }());
      ...
      return UpdatePermissionsRequest(
        keys: json[r'keys'] is Iterable
            ...
            : const [],
        data: CreatePermissionRequest.fromJson(json[r'data']),
      );
    }
    return null;
  }
