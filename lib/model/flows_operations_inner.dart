...
class FlowsOperationsInner {
  /// Returns a new [FlowsOperationsInner] instance.
  ...
  @override
  bool operator ==(Object other) => identical(this, other) || other is FlowsOperationsInner &&
    other.id == id &&
    other.name == name &&
    other.key == key &&
    other.type == type &&
  ...
  @override
  String toString() => 'FlowsOperationsInner[id=$id, name=$name, key=$key, type=$type, positionX=$positionX, positionY=$positionY, options=$options, resolve=$resolve, reject=$reject, flow=$flow, dateCreated=$dateCreated, userCreated=$userCreated]';
  ...
  /// Returns a new [FlowsOperationsInner] instance and imports its values from
  /// [value] if it's a [Map], null otherwise.
  // ignore: prefer_constructors_over_static_methods
  static FlowsOperationsInner? fromJson(dynamic value) {
    if (value is Map) {
      ...
      assert(() {
        requiredKeys.forEach((key) {
          assert(json.containsKey(key), 'Required key "FlowsOperationsInner[$key]" is missing from JSON.');
          assert(json[key] != null, 'Required key "FlowsOperationsInner[$key]" has a null value in JSON.');
        });
        return true;
      }());

      return FlowsOperationsInner(
  ...
  static List<FlowsOperationsInner> listFromJson(dynamic json, {bool growable = false,}) {
    final result = <FlowsOperationsInner>[];
    if (json is List && json.isNotEmpty) {
      for (final row in json) {
        final value = FlowsOperationsInner.fromJson(row);
        if (value != null) {
          result.add(value);
        }
      }
  ...
  static Map<String, FlowsOperationsInner> mapFromJson(dynamic json) {
    final map = <String, FlowsOperationsInner>{};
    if (json is Map && json.isNotEmpty) {
      json = json.cast<String, dynamic>(); // ignore: parameter_assignments
      for (final entry in json.entries) {
        final value = FlowsOperationsInner.fromJson(entry.value);
        if (value != null) {
          map[entry.key] = value;
        }
      }
  ...
  // maps a json object with a list of FlowsOperationsInner-objects as value to a dart map
  static Map<String, List<FlowsOperationsInner>> mapListFromJson(dynamic json, {bool growable = false,}) {
    final map = <String, List<FlowsOperationsInner>>{};
    if (json is Map && json.isNotEmpty) {
      // ignore: parameter_assignments
      json = json.cast<String, dynamic>();
      for (final entry in json.entries) {
        map[entry.key] = FlowsOperationsInner.listFromJson(entry.value, growable: growable,);
      }
    }
    return map;
  }
