...
class Revisions {
  ...
  @override
  bool operator ==(Object other) => identical(this, other) || other is Revisions &&
    other.id == id &&
  ...
  @override
  String toString() => 'Revisions[id=$id, activity=$activity, collection=$collection, item=$item, data=$data, delta=$delta, parent=$parent, version=$version]';
  ...
  static Revisions? fromJson(dynamic value) {
    if (value is Map) {
      ...
      assert(() {
        requiredKeys.forEach((key) {
          assert(json.containsKey(key), 'Required key "Revisions[$key]" is missing from JSON.');
          assert(json[key] != null, 'Required key "Revisions[$key]" has a null value in JSON.');
        });
        return true;
      }());
  ...
  static List<Revisions> listFromJson(dynamic json, {bool growable = false,}) {
  ...
  static Map<String, List<Revisions>> mapListFromJson(dynamic json, {bool growable = false,}) {
    final map = <String, List<Revisions>>{};
    if (json is Map && json.isNotEmpty) {
      // ignore: parameter_assignments
      json = json.cast<String, dynamic>();
      for (final entry in json.entries) {
        map[entry.key] = Revisions.listFromJson(entry.value, growable: growable,);
      }
    }
    return map;
  }
