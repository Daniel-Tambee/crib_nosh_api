...
class Versions {
  ...
  @override
  String toString() => 'Versions[id=$id, key=$key, name=$name, collection=$collection, item=$item, hash=$hash, dateCreated=$dateCreated, dateUpdated=$dateUpdated, userCreated=$userCreated, userUpdated=$userUpdated, delta=$delta]';
  ...
  static Map<String, List<Versions>> mapListFromJson(dynamic json, {bool growable = false,}) {
    ...
    if (json is Map && json.isNotEmpty) {
      ...
      for (final entry in json.entries) {
        map[entry.key] = Versions.listFromJson(entry.value, growable: growable,);
      }
    }
    return map;
  }
