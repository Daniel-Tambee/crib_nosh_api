...
class Fields {
  ...
  @override
  String toString() => 'Fields[id=$id, collection=$collection, field=$field, special=$special, interface_=$interface_, options=$options, display=$display, displayOptions=$displayOptions, readonly=$readonly, hidden=$hidden, sort=$sort, width=$width, translations=$translations, note=$note, conditions=$conditions, required_=$required_, group=$group, validation=$validation, validationMessage=$validationMessage]';
    ...
    if (value is Map) {
      ...
      assert(() {
        requiredKeys.forEach((key) {
          assert(json.containsKey(key), 'Required key "Fields[$key]" is missing from JSON.');
          assert(json[key] != null, 'Required key "Fields[$key]" has a null value in JSON.');
        });
        return true;
      }());
