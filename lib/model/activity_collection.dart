...
class ActivityCollection {
  /// Returns a new [ActivityCollection] instance.
  ActivityCollection({
    this.collection,
    this.icon,
    this.note,
  ...
  @override
  bool operator ==(Object other) => identical(this, other) || other is ActivityCollection &&
  ...
  @override
  String toString() => 'ActivityCollection[collection=$collection, icon=$icon, note=$note, displayTemplate=$displayTemplate, hidden=$hidden, singleton=$singleton, translations=$translations, archiveField=$archiveField, archiveAppFilter=$archiveAppFilter, archiveValue=$archiveValue, unarchiveValue=$unarchiveValue, sortField=$sortField, accountability=$accountability, color=$color, itemDuplicationFields=$itemDuplicationFields, sort=$sort, group=$group, collapse=$collapse, previewUrl=$previewUrl, versioning=$versioning]';
  ...
  /// Returns a new [ActivityCollection] instance and imports its values from
  /// [value] if it's a [Map], null otherwise.
  // ignore: prefer_constructors_over_static_methods
  static ActivityCollection? fromJson(dynamic value) {
    if (value is Map) {
      ...
      assert(() {
        requiredKeys.forEach((key) {
          assert(json.containsKey(key), 'Required key "ActivityCollection[$key]" is missing from JSON.');
          assert(json[key] != null, 'Required key "ActivityCollection[$key]" has a null value in JSON.');
        });
        return true;
      }());
  ...
  static List<ActivityCollection> listFromJson(dynamic json, {bool growable = false,}) {
    final result = <ActivityCollection>[];
    if (json is List && json.isNotEmpty) {
      for (final row in json) {
        final value = ActivityCollection.fromJson(row);
        if (value != null) {
          result.add(value);
        }
      }
  ...
  static Map<String, ActivityCollection> mapFromJson(dynamic json) {
    final map = <String, ActivityCollection>{};
    if (json is Map && json.isNotEmpty) {
      json = json.cast<String, dynamic>(); // ignore: parameter_assignments
      for (final entry in json.entries) {
        final value = ActivityCollection.fromJson(entry.value);
        if (value != null) {
          map[entry.key] = value;
        }
      }
  ...
  // maps a json object with a list of ActivityCollection-objects as value to a dart map
  static Map<String, List<ActivityCollection>> mapListFromJson(dynamic json, {bool growable = false,}) {
    final map = <String, List<ActivityCollection>>{};
    if (json is Map && json.isNotEmpty) {
      // ignore: parameter_assignments
      json = json.cast<String, dynamic>();
      for (final entry in json.entries) {
        map[entry.key] = ActivityCollection.listFromJson(entry.value, growable: growable,);
      }
    }
    return map;
  }
