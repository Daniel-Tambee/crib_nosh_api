...
class OperationsReject {
  /// Returns a new [OperationsReject] instance.
  OperationsReject({
    this.id,
    this.name,
    this.key,
  ...
  @override
  bool operator ==(Object other) => identical(this, other) || other is OperationsReject &&
    other.id == id &&
    other.name == name &&
    other.key == key &&
    other.type == type &&
  ...
  @override
  String toString() => 'OperationsReject[id=$id, name=$name, key=$key, type=$type, positionX=$positionX, positionY=$positionY, options=$options, resolve=$resolve, reject=$reject, flow=$flow, dateCreated=$dateCreated, userCreated=$userCreated]';
  ...
  /// Returns a new [OperationsReject] instance and imports its values from
  /// [value] if it's a [Map], null otherwise.
  // ignore: prefer_constructors_over_static_methods
  static OperationsReject? fromJson(dynamic value) {
    if (value is Map) {
      ...
      assert(() {
        requiredKeys.forEach((key) {
          assert(json.containsKey(key), 'Required key "OperationsReject[$key]" is missing from JSON.');
          assert(json[key] != null, 'Required key "OperationsReject[$key]" has a null value in JSON.');
        });
        return true;
      }());

      return OperationsReject(
  ...
  static List<OperationsReject> listFromJson(dynamic json, {bool growable = false,}) {
    final result = <OperationsReject>[];
    if (json is List && json.isNotEmpty) {
      for (final row in json) {
        final value = OperationsReject.fromJson(row);
        if (value != null) {
          result.add(value);
        }
      }
  ...
  static Map<String, OperationsReject> mapFromJson(dynamic json) {
    final map = <String, OperationsReject>{};
    if (json is Map && json.isNotEmpty) {
      json = json.cast<String, dynamic>(); // ignore: parameter_assignments
      for (final entry in json.entries) {
        final value = OperationsReject.fromJson(entry.value);
        if (value != null) {
          map[entry.key] = value;
        }
      }
  ...
  // maps a json object with a list of OperationsReject-objects as value to a dart map
  static Map<String, List<OperationsReject>> mapListFromJson(dynamic json, {bool growable = false,}) {
    final map = <String, List<OperationsReject>>{};
    if (json is Map && json.isNotEmpty) {
      // ignore: parameter_assignments
      json = json.cast<String, dynamic>();
      for (final entry in json.entries) {
        map[entry.key] = OperationsReject.listFromJson(entry.value, growable: growable,);
      }
    }
    return map;
  }
